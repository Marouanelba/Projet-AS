...
const indicateurExample = `{
  "code": "2 - 1",
  "thematique_code": "2",
  "annuaire_annee": "2024",
  "titre_fr": "Population totale par région",
  "titre_ar": "مجموع السكان حسب الجهة",
  "unite": {"fr": "milliers", "ar": "بالآلاف"},
  "source": {"fr": "HCP - Recensement 2024", "ar": "المندوبية السامية للتخطيط"},
  "notes": {"fr": "(1) Estimations provisoires"},
  "indices": {
    "(1)": {"signification_fr": "Données provisoires", "rattache_type": "donnees"}
  },
...
const ImportData = () => {
  ...
  const downloadTemplate = (type: 'metadata' | 'indicateur') => {
    const content = type === 'metadata' ? metadataExample : indicateurExample;
    const filename = type === 'metadata' ? 'metadata.json' : 'indicateur_exemple.json';
          ...
          <TabsContent value="format" className="space-y-6">
            ...
            <div className="border-2 border-slate-200 rounded-2xl overflow-hidden">
              <div className="bg-emerald-50 px-5 py-3 border-b border-emerald-100 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-600 text-white">TYPE 2</span>
                  <span className="text-sm font-bold text-slate-900">indicateur_X___Y.json</span>
                </div>
                <Button variant="outline" size="sm" onClick={() => downloadTemplate('indicateur')} className="rounded-lg text-xs gap-1.5 border-emerald-200 text-emerald-700 hover:bg-emerald-100">
                  <Download className="h-3.5 w-3.5" /> Télécharger l'exemple
                </Button>
              </div>
              <div className="p-4">
                <p className="text-xs text-slate-500 mb-3">Fichier contenant un tableau statistique avec ses données :</p>
                <div className="rounded-xl bg-slate-900 p-4 overflow-x-auto">
                  <pre className="text-xs font-mono text-emerald-400 whitespace-pre">{indicateurExample}</pre>
                </div>
              </div>
            </div>
          </TabsContent>
