...
export default function DataTableWithExport({
  ...
  const hasMergedCells = useMemo(() => {
    return mergedCells && mergedCells.length > 0;
  }, [mergedCells]);
  ...
  const mergeGrid = useMemo(() => {
    const grid: Record<string, { hidden: boolean; rowSpan?: number; colSpan?: number }> = {};
    if (!mergedCells || mergedCells.length === 0) return grid;
  ...
  const renderTableBody = () => (
    <tbody>
      {reorderedDonnees.map((row, rowIndex) => {
        const globalRowIdx = entetes.length + rowIndex;
        return (
          <tr key={`row-${rowIndex}`} className="hover:bg-muted/50">
            {Array.isArray(row) &&
              row.map((cell, cellIndex) => {
                const origColIdx = columnOrder[cellIndex];
                const mergeKey = `${globalRowIdx}-${origColIdx}`;
          ...
          {reorderedEntetes.map((row, rowIndex) => (
            <tr key={`header-${rowIndex}`} className="bg-muted">
              {Array.isArray(row) &&
                row.map((cell, cellIndex) => {
                  const origColIdx = columnOrder[cellIndex];
                  const mergeKey = `${rowIndex}-${origColIdx}`;
                  const merge = mergeGrid[mergeKey];
                  if (merge?.hidden) return null;
                  return (
                    <th
