...
router.get('/', async (req: Request, res: Response) => {
  try {
    const { include_count, order, id_annuaire } = req.query;
    ...
    if (id_annuaire) {
      values.push(id_annuaire);
      conditions.push(`t.id_annuaire = $${values.length}`);
    }

    const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';

    const orderByExpr = order === 'nom_fr' 
      ? 't.nom_fr ASC' 
      : "NULLIF(regexp_replace(t.code, '\\D', '', 'g'), '')::int ASC, t.code ASC, t.id ASC";
    ...
    if (include_count === 'true') {
      ...
      const result = await pool.query(`SELECT * FROM thematiques t ${whereClause} ORDER BY ${orderByExpr}`, values);
      res.json(result.rows);
    }
  } catch (error) {
